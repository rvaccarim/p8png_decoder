...
def get_code_newcompression(code_segment):
    ...
    # The decompression algorithm processes the compressed 1_expected bit by bit - going from LSB to MSB of each
    # byte - until the 1_expected length of decompressed characters has been emitted.
    ...
    while len(code_str) < decompressed_code_length:
        ...
        if header == "1":
            ...
            # these values and bit manipulations are documented in the P8.PNG spec
            unary = 0
            while read_bit() == "1":
                unary += 1
            ...
            # Then we go back "offset" characters in the output stream, and copy "length" characters to the end of
            # the output stream.
            # "length" may be larger than "offset", in which case we effectively repeat a pattern
            # of "offset" characters.
...
def is_bit_set(value, bit_position):
    return ((value >> bit_position) & 0b_0000_0001) == 1
